...
public final class FFNN {
    ...
    public init(inputs: Int, hidden: Int, outputs: Int, learningRate: Float = 0.7, momentum: Float = 0.4, weights: [Float]? = nil, activationFunction: ActivationFunction = .Default, errorFunction: ErrorFunction = .default(average: false)) {
        ...
        self.hiddenWeights = [Float](repeating: 0.0, count: self.numHiddenWeights)
    ...
    public func backpropagate(answer: [Float]) throws -> Float {
        ...
        if activationFunction == .MetalSigmoid {
            BenchmarkTimer.start(label: "Metal start")
            FFNNMetal.instance.updateHiddenWeight(hiddenWeights: self.hiddenWeights,
                                                  previousHiddenWeights: self.previousHiddenWeights,
                                                  hiddenErrorIndices: self.hiddenErrorIndices,
                                                  inputIndices: self.inputIndices,
                                                  inputCache: self.inputCache,
                                                  mfLR: self.mfLR,
                                                  momentumFactor: self.momentumFactor,
                                                  hiddenOutputCache: self.hiddenOutputCache,
                                                  hiddenErrorsCache: self.hiddenErrorsCache,
                                                  newHiddenWeights: &self.newHiddenWeights)
            BenchmarkTimer.stop()
        }else {
            
            
            //BenchmarkTimer.start(label: "Concurrent start")
            let numberOfThread = 1
            let iterationSize = self.hiddenWeights.count/numberOfThread
            DispatchQueue.concurrentPerform(iterations: numberOfThread, execute: { (i) in
                
                    for weightIndex in iterationSize*i..<iterationSize*(i+1) {
            ...
            /*
            BenchmarkTimer.start(label: "Classic start")
...
public extension FFNN {
    ...
    func fromData(_ data:Data) -> FFNN? {
    ...
    fileprivate func activateHidden() {
        ...
        case .Linear:
            ...
            self.hiddenOutputCache[0] = 1.0
        case .Sigmoid, .Softmax, .MetalSigmoid:
